<x-dashboard-layout>
    <div class="mt-14 flex items-center justify-between">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ $product ? __('Edit Produk') : __('Tambah Produk Baru') }}
        </h2>
    ...
    <div class="flex mt-4">
        <form id="product-form" action="{{ $product ? route('product.update', $product->slug) : route('product.store') }}"
            method="POST" enctype="multipart/form-data" class="flex flex-wrap gap-4 w-full max-w-screen-xl">
            @csrf
            @if ($product)
                @method('PUT')
            @endif
                ...
                <div class="mb-4 p-4 bg-white border border-gray-300 rounded-md">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="categories" name="categories[]" required
                            class="border border-gray-300 shadow-sm text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            ...
            <div class="flex flex-col w-full lg:w-1/4">
                <div class="mb-4">
                    ...
                    <x-input.text type="number" id="price" name="price"
                        value="{{ old('price', $product->price ?? '') }}" inputmode="numeric" pattern="\d*"
                        required />
